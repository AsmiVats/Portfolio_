        ...
        <div className="container mt-4 mx-auto p-4 flex flex-col items-center">
            ...
            <ul className="grid grid-cols-1 gap-[4vw] w-full justify-center items-center">
                {cards.map((card, index) => (
                    ...
                    >
                        <div
                            className={`card-body ${card.bgColor} flex rounded-3xl shadow-lg transition-all duration-500 p-8 ${card.size} flex justify-center items-center transform hover:scale-105 animate-on-scroll`}
                        >
                            <div className="flex flex-col justify-center w-1/2 text-left">
                                <h2 className="text-white text-[24px] font-bold mb-4 font-[degular-display]">{card.title}</h2>
                                <p className="text-white text-lg mb-4 font-[manrope]">{card.description}</p>

                                {/* Features as badges */}
                                <div className="flex flex-col flex-wrap gap-2 mb-4">
                                    {card.features.map((feature, i) => (
                                        <span key={i} className="bg-white text-black text-[14px] px-3 py-1 rounded-full shadow-md w-3/4">{feature}</span>
                                    ))}
                                </div>

                                {/* Tech Stack */}
                                <p className="text-white text-sm mb-2 font-semibold">Tech Stack:</p>
